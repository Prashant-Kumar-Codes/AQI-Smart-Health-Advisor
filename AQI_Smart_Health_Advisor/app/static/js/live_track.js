...
async function updateLocationDisplay(lat, lng, accuracy) {
    try {
        // Use reverse geocoding with precise coordinates
        ...
        document.getElementById('locationName').textContent = 'Location detected';
        document.getElementById('locationCoords').textContent = 
            `${lat.toFixed(6)}, ${lng.toFixed(6)} (±${accuracy.toFixed(0)}m)`;
    }
}
...
function updateLastUpdateTime() {
    const now = new Date();
    document.getElementById('lastUpdate').textContent = formatTimestamp(now);
}
...
function updateCurrentAQIDisplay(data) {
    ...
    document.getElementById('aqiCategory').textContent = category.category;
...
function updateTrackingUI() {
    ...
    if (trackingActive) {
        ...
        document.getElementById('locationName').textContent = 'Not tracking';
        document.getElementById('locationCoords').textContent = '';
    }
}
